...
/*   Updated: 2022/04/19 05:08:25 by yanab            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (stack_b->size > 0)
	{
		...
		else
		{
			...
			else
			{
				...
				else if (unordered_count == 0 && stack_b->top->content < get_stack_tail(*stack_a)->content)
				{
					p(stack_a, stack_b, TRUE, 'a');
					r(stack_a, TRUE, 'a');
					unordered_count += 1;
...
void print_merged_ops(char *ops_str)
{
	...
	{
		if (
			(!ft_strcmp(ops[i], "rb") && !ft_strcmp(ops[i + 1], "ra")) || (!ft_strcmp(ops[i], "ra") && !ft_strcmp(ops[i + 1], "rb")))
		{
			write(1, "rr\n", 3);
			i += 2;
		}
